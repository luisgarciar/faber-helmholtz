...
%   - multi-grid
%
% The 2D Dirichlet problem is discretized with finite differences ???.
%
% Further, we do the same when applying the Faber preconditioner s_n:
%   AM^{-1} s_n(AM^{-1}) x = b.

% Computation: this file
% Postprocessing: mp_2ddir_postprocess
...
degree  = 1:2;
wavenum = [5, 10, 15, 20, 40, 60, 80];
wavenum = [5, 10, 15, 20];

%Parameters for GMRES
restart = [];
tol     = 1e-8;
maxit   = 350;  %maximum number of gmres cycles
...
save('mp_2ddir_results', 'wavenum', 'dim', 'degree', 'restart', 'bc',...
