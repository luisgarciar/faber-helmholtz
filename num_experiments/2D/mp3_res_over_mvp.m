...
% MP3: First experiment in paper: k=50 or k=100, each with restart=20 and []
k = 50;  restart = 20;
% k = 50;  restart = [];
% k = 100; restart = 20;
% k = 100; restart = [];
...
figure(1)
semilogy(mvops_mg, resvec_mg, 'k-', LW, lw)
hold on
...
for deg = 1:length(degree)
    ...
    for t=1:numruns
        ...
        semilogy(mvops_mg_FS, resvec_mg_FS, 'color',linecolor{deg},...
            'linestyle',linestyle{deg}, LW, lw)
        hold on
        ylim([1e-8, 1])
        
        leg{deg+1} = strcat('FP(',num2str(deg),')+CSL(MG)');
        
        legend(leg,'Location','NorthEast','FontSize',16,'interpreter','latex')
    end
...
if ( save_flag == 1 )
    ...
    matlab2tikz('filename',plot_file_tex, 'width', '\figWidth', ...
        ...
        'ylabel style={font=\scriptsize, at={(axis description cs:-0.12,.5)},anchor=south},', ...
