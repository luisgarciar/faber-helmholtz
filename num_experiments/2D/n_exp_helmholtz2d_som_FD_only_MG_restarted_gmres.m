...
mvop_mg     = (iter_mg(:,1)*restart)+iter_mg(:,2);
mvop_mg_FS  = (1 + deg)*(iter_mg_FS(:,1)*restart+iter_mg_FS(:,2));
...
plot_time_tex = strcat('exp1_restart_time_vs_k_',dimn,'D_',...
    bdc,'.tex');
...
if ( save_flag == 1 )
    ...
    plot_time_eps = strcat('exp1_restart_time_vs_k_',dimn,'D_',bdc,'.eps');
...
figure(2)
plot(wavenum,(iter_mg(:,1)*restart)+iter_mg(:,2), 'k-')
hold on
plot(wavenum,(iter_mg_FS(:,1)*restart)+iter_mg_FS(:,2), 'b--')
hold off
