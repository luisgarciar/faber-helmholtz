...
%   - multi-grid
%
% The 2D Sommerfeld problem is discretized with finite elements
%
...
for kk = 1:length(wavenum)
    ...
    [npf,numlev] = fem_npc_to_npf(npcc,k,ppw);
    ...
    [mg_mat_helm,~,~,~]= mg_setupfem_2D(npcc,numlev,pdehelm);
    
    A = mg_mat_helm{1}; Aeps = mg_mat_SL{1};
...
%title(['2D Helmholtz with CSL-preconditioner ($k=$',num2str(k),') and deg=',...
%    num2str(deg)])
...
legend('CSL (LU)',...
    'CSL (MG)', ...
    'FP+ CSL (LU),  deg=1', ...
    'FP + CSL (MG), deg=1','FontSize',FS,'Location','NorthEast')
...
table_data = [wavenum.',iter_lu(:,2),time_lu, mvop_lu,...
    iter_lu_FS(:,2),time_lu_FS, mvop_lu_FS,...
    iter_mg(:,2),time_mg, mvop_mg,...
    iter_mg_FS(:,2),time_mg_FS,mvop_mg_FS];

numrows = size(table_data,1);
numcols = size(table_data,2);
tableCaption = strcat('Comparison of number of GMRES iterations, timings and matrix-times-vector operations for 1D Sommerfeld problem');
...
row1   = {'$k$ & \multicolumn{3}{c}{CSL+LU} & \multicolumn{3}{c}{FP+CSL+LU} & ',...
    '\multicolumn{3}{c}{CSL+MG} & \multicolumn{3}{c}{FP+CSL+MG}\\'};
row2  = {' & Iter. & Time(s) & MatVecs  &  Iter. & Time(s) & Matvecs & ', ...
    '   Iter. & Time(s) & MatVecs  &  Iter. & Time(s) & Matvecs \\ \midrule'};
