...
for s = 1:numruns
    for kk = 1:length(wavenum)
        ...
        npc       = 1;   %number of points in the coarsest grid in 1D
        ppw       = 15;  %pollution free grid
        bc        = 'som1';
        %number of points in finest grid and number of levels
        [npf,numlev] = fd_npc_to_npf(npc,2*kref,ppw);
        ...
        op_type = 'gal';   
        [mg_mat_SL,mg_split,restr,interp] = mg_setup_kvar(kref,factoreps,poweps,op_type,npc,numlev,bc,dim);
...
plot_time_tex = strcat('wedge_time_vs_k_',kmin,'to',kmax,...
...
if ( save_flag == 1 )
    ...
    plot_time_eps = strcat('wedge_time_vs_k_',kmin,'to',kmax,...
        '_',dimn,'D_',bdc,'deg',degmin,'to',degmax,...
        'restart',restt,'.eps');
...
%hold on
%plot(wavenum, totiter_mg_FS(3,:), 'r-');
hold off
...
plot_iter_tex = strcat('wedge_iter_vs_k_',kmin,'to',kmax,...
    '_',dimn,'D_',bdc,'deg',degmin,'to',degmax,...
    'restart',restt,'.tex');
...
if ( save_flag == 1 )
    ...
    %Save the plot as .eps
    plot_iter_eps = strcat('wedge_iter_vs_k_',kmin,'to',kmax,...
        '_',dimn,'D_',bdc,'deg',degmin,'to',degmax,...
        'restart',restt,'.eps');
...
tableCaption = strcat('Model problem 4: Number of GMRES iterations, matrix-vector products and timings, computing the shifted Laplacian with a MG cycle.');
tableLabel   = strcat('wedge_mg_','restart',restt,'_',dimn,'D_',bdc);
...
row1   = {'$k_{\mathrm{ref}}$ & \multicolumn{3}{c}{CSL(MG)} & \multicolumn{3}{c}{FP(1)+CSL(MG)} & ',...
...
namefile = strcat('wedge_table_mg_','restart',restt,'_',dimn,'D_',bdc,'.tex');
currentpath = mfilename('fullpath');
