% CSL-preconditioned 1D Helmholtz with Dirichlet boundary conditions
% 
...
wavenum = [20:20:100,200:200:1000];
...
hold on
%plot(wavenum, time_lu_FS, 'k-', LW, lw)
plot(wavenum, time_ilu_FS(:,1), 'b-', LW, lw)
%plot(wavenum, time_mg_FS(:,1), 'b-', LW, lw)

plot(wavenum, time_ilu_FS(:,2), 'b--', LW, lw)
...
legend('CSL (iLU(0))', 'FS + CSL (iLU(0)), deg=1','FS + CSL(iLU(0)), deg=2', ...
...
fs =18;
figure(2);
%plot(wavenum, iter_lu, 'k-', LW, lw)
%hold on
%plot(wavenum, iter_ilu(:,2), 'k-.', LW, lw)
%plot(wavenum, iter_lu_FS(:,1), 'b-', LW, lw)
%plot(wavenum, iter_ilu_FS, 'b-.', LW, lw)

%All ilU
plot(wavenum, iter_ilu, 'k-', LW, lw)
hold on
plot(wavenum, iter_ilu_FS(:,1), 'b-', LW, lw)
plot(wavenum, iter_ilu_FS(:,2), 'b--', LW, lw)
...
% ylabel('Number of GMRES iterations', 'FontSize',fs)
% xlabel('Wavenumber','FontSize',fs)
% legend('CSL (MG)','FS + CSL (MG), deg=1','FS + CSL(MG), deg=2') ...
%     
...
tableCaption = strcat('Comparison of number of GMRES iterations and timings for 1D Dirichlet problem');
